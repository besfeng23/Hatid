...
function RiderSavedPlacesPage() {
  ...
  return (
    <RiderScreenContainer className="pb-24">
      <div className="mx-auto max-w-[430px]">
        <RiderTopBar showBack title="Saved places" subtitle="Home, work, and favorites" />
        <div className="space-y-5 px-4">
          <LocationSearchField value={search} onChange={setSearch} placeholder="Search saved places" />
          <SavedPlacesSection title="Favorites" places={favorites} editable />
          <SavedPlacesSection title="Other saved places" places={others} editable />
          <Button size="lg" className="w-full" disabled>Add new saved place (not active yet)</Button>
          <InlineTrustBanner>Editing controls are visual only. New saved-place writes remain limited to existing data paths until backend rules are finalized.</InlineTrustBanner>
        </div>
        <div className="fixed inset-x-0 bottom-0"><RiderBottomNav active="Saved" /></div>
      </div>
